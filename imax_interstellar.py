...
while True:
	...
	# 영화제목이 크롤링 데이터 내에 있는지 확인후 메세지 전송
		...
		if str('인터스텔라') in html:
			bot.sendMessage(chat_id = -1001299873599, text = "%d 인터스텔라 예매오픈!" % date) 
			confirm_date = date
		print(now)
		print('date =', date)
		print('정상작동중')
	...
	# 매일 1회 정상작동 메세지 출력, 이후 1분간 휴식
		...
		if(now.hour == 12 and now.minute == 40):
			bot.sendMessage(chat_id = -1001299873599, text = "%d:%d 용아맥알리미 인터스텔라 정상작동중!" % (now.hour, now.minute))
			time.sleep(60)
		print(now)
		print('date =', date)
		print('정상작동중')
